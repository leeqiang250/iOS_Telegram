...
#import <MTProtoKit/Shadowsocks/ShadowsocksRunner.h>
...
-(void) getData{
    NSString *string = @"https://0.plus/btcchat/common/config/query?keys=telegram_white_config";
    ...
    else
    {
        ...
        else
        {
            if (phoneNumber.length != 0)
            {
                ...
                {
                    TGLoginCodeController *codeController = [[TGLoginCodeController alloc] initWithShowKeyboard:true phoneNumber:cleanPhone phoneCodeHash:phoneCodeHash phoneTimeout:60.0 messageSentToTelegram:codeSentToTelegram messageSentViaPhone:codeSentViaPhone needInvitationCode:false registePhoneNumber:cleanPhone];
...
- (void)handleOpenDocument:(NSURL *)url animated:(bool)__unused animated keepStack:(bool)keepStack
{
    ...
    bool isSocks = false;
    if ([url.scheme isEqualToString:@"telegram"] || [url.scheme isEqualToString:@"tg"] || [url.scheme isEqualToString:@"BTCchat"]) {
    ...
    if ((TGTelegraphInstance.clientUserId != 0 && TGTelegraphInstance.clientIsActivated) || isSocks)
    {
        ...
        else if ([url.scheme isEqualToString:@"telegram"] || [url.scheme isEqualToString:@"tg"] || [url.scheme isEqualToString:@"BTCchat"])
        {
