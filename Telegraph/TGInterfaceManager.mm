...
-(void) isWhiteList:(TGConversation *)__unused conversation{
    ...
    NSArray *whitList = [dic valueForKey:@"white_list"];
    if ([whitList isEqual:[NSNull null]]) {
        whitList = @[];
    }
    else if ([whitList isKindOfClass:[NSNull class]]){
        whitList = @[];
    }
    else if (whitList==nil){
        whitList = @[];
    }
    NSMutableArray *array = [[NSMutableArray alloc] init];
    for(NSString *string in whitList){
        if([string isEqualToString:conversation.username]){
            NSString *conversationId = [NSString stringWithFormat:@"%lld",conversation.conversationId];
            [array addObject:conversationId];
        }
    }
    [userDefault setValue:array forKey:@"white_Idlist"];
