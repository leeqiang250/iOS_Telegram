...
- (void)updateSelectionInterface
{
    ...
    if (count != 0)
    {
        if (_inviteContainer == nil)
        {
            ...
            inviteLabel.text = TGLocalized(@"Contacts.InviteToBTCchat");
...
- (void)updateContactList
{
    ...
    // 移除 新建群组，新建私密对话，新的频道
    else if (false && ((_contactsMode & TGContactsModeMainContacts) == TGContactsModeMainContacts || ((_contactsMode & TGContactsModeCreateGroupOption) == TGContactsModeCreateGroupOption) || ((_contactsMode & TGContactsModeModalInvite) == TGContactsModeModalInvite)))
    {
        std::shared_ptr<TGContactListSection> serviceSection(new TGContactListSection());
