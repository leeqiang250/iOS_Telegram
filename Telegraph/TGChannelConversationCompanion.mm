...
-(BOOL) isWhiteList:(int64_t) conversationId{
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    NSArray *array = [userDefault valueForKey:@"white_Idlist"];
    NSString *conversationIdString = [NSString stringWithFormat:@"%lld",conversationId];
    ...
    for(NSString *string in array){
        if([string isEqualToString:conversationIdString]){
            return YES;
        }
    }
    return NO;
...
- (void)requestJoinChannel {
    //在搜索出过滤白名单，加入时不再判定
    /*if(![self isWhiteList:_conversationId]){
        return;
    }*/
