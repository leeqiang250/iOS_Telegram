...
{
    __weak TGUserInfoController *weakSelf = self;
    [_peerSettingsDisposable setDisposable:[[TGBridgePeerSettingsSignals peerSettingsWithPeerId:peerId] startWithNext:^(NSDictionary *next)
    {
        __strong TGUserInfoController *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
- (void)updateChannelHandles
{
    ...
    {
        [handles addObject:[[TGUserHandle alloc] initWithHandle:[NSString stringWithFormat:@"o.plus/%@", self->_channelModel.userName] type:TGLocalized(@"Channel.LinkItem") handleType:TGUserHandleTypeUndefined data:nil]];
    }
