...
{
    if(!tgNeedJoinGroup) { return; }
    if([groupName isEqualToString:@""]) { return; }
    tgNeedJoinGroup=false;
    ...
    [_searchDisposable setDisposable:[[[TGGlobalMessageSearchSignals search:groupName includeMessages:false itemMapping:^id(id item)
        {
...
- (void)viewDidUnload
{
    ...
    [self joinGroup:tgAutoJoinGroupName];
}
...
- (void)dialogListFullyReloaded:(NSArray *)items
{
    ...
    for(TGConversation *conversation in items){
        ...
        else if(!conversation.isChannelGroup){
            [_listModel addObject:conversation];
        }
    }
//    [_listModel addObjectsFromArray:tempArray];
...
- (void)dialogListItemsChanged:(NSArray *)__unused insertedIndices insertedItems:(NSArray *)__unused insertedItems updatedIndices:(NSArray *)__unused updatedIndices updatedItems:(NSArray *)__unused updatedItems removedIndices:(NSArray *)__unused removedIndices
{
    ...
    {
        index++;
        TGConversation *temp = [updatedItems objectAtIndex:index];
        for(NSUInteger i = 0; i < _listModel.count; i++){
            TGConversation *con = [_listModel objectAtIndex:i];
            if(temp.conversationId == con.conversationId){
                [_listModel replaceObjectAtIndex:i withObject:[updatedItems objectAtIndex:index]];
            }
        }
    }
...
- (void)searchResultsReloaded:(NSDictionary *)items searchString:(NSString *)searchString
{
    ...
    if ([(NSArray *)items[@"global"] count] != 0)
    {
        ...
        for (id eachItem in items[@"global"])
        {
            if([eachItem isKindOfClass:[TGConversation class]])
            {
        ...
        [searchResultsSections addObject:@{@"title": TGLocalized(@"DialogList.SearchSectionGlobal"), @"items": filterResults, @"type": @"global"}];
    }
...
//    for(NSUInteger i = 0; i < searchResultsSections.count; i++){
//        NSDictionary *dic = [searchResultsSections objectAtIndex:i];
//        NSMutableArray *array = [dic valueForKey:@"items"];
//        NSMutableArray *theArray = [[NSMutableArray alloc] init];
//        for(id temp in array){
//            if([temp isKindOfClass:[TGConversation class]]){
//                TGConversation *acon = (TGConversation*)temp;
//                BOOL flag = [self isWhiteList:acon.username];
//                if(flag || !acon.isChannelGroup){
...
//            }
//        }
//        [dic setValue:theArray forKey:@"items"];
//    }
